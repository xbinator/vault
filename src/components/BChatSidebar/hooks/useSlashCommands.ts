...
export const chatSlashCommands: SlashCommandOption[] = (
  [
    { id: 'model', title: '模型', description: '切换当前使用的模型。' },
    { id: 'usage', title: '使用情况', description: '显示当前会话的 token 使用情况。' },
    { id: 'new', title: '新建聊天', description: '开始一个新的聊天会话。' },
    { id: 'clear', title: '清空输入', description: '清除当前聊天输入内容。' },
    { id: 'compact', title: '压缩上下文', description: '立即执行一次手动上下文压缩。' }
  ] satisfies Array<{ id: CommandId; title: string; description: string }>
).map(({ id, title, description }) => ({ id, trigger: `/${id}`, title, description, type: 'action' as const }));
...
export function useSlashCommands(handlers: CommandHandlers) {
   ...
   * @param command - 斜杠命令项
   */
  function handleSlashCommand({ id }: SlashCommandOption): void {
